...
class MainMenu:
    ...
    def write(self):
        for key in self.keys + ["backspace"]:
            if keyboard.is_pressed(key) and self.t > 4 and (not keyboard.is_pressed("shift")) and (key != "backspace"):
                if not self.PlaceActive and len(self.Nikname) < 15 and not key.isnumeric():
                    self.Nikname += key
                elif self.PlaceActive and len(self.Password) < 10:
                    self.Password += key
                self.t = 0
            elif keyboard.is_pressed("shift+" + key) and self.t > 4 and (key != "backspace"):
                if not self.PlaceActive and len(self.Nikname) < 10:
                    self.Nikname += key.upper()
                elif self.PlaceActive and len(self.Password) < 10:
                    self.Password += key.upper()
                self.t = 0
            elif keyboard.is_pressed(key) and self.t > 4 and key == "backspace":
